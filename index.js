...
Tokenize.prototype._transform = function(buf, enc, next) {
  ...
  for(var i = this._position; i < input.length; i++) {
    ...
    && c === codes.semi) {
      ...
      i--;
      this.state.push(['atrule_end']);
    }
    else if('atrule_start' === state
    && c === codes.lbrace) {
      end = this.state.pop();
      this.state.push(['rule_start']);//this.state.push(['atrule']);
    }
    else if('atrule_start' === state
    && c === codes.rbrace) {
      end = this.state.pop();
      i--;
      this.state.push(['atrule_end']);//this.state.push(['atrule']);
    }
    else if(('rule' === state || 'atrule' === state)
    && c === codes.semi) {
      if (input.toString().lastIndexOf('data:',i)==-1 || (input.toString().lastIndexOf('data:',i)<input.toString().lastIndexOf(')',i))) {
        end = this.state.pop();
    ...
    {
      if('root' === state && c === codes.rbrace) {
        end = ['root'];
        i--;
        this.state.push(['rule_end'])
      }
      else if('root' === state) {
        end = ['root'];
        i--;
